...
public interface RetweetRepository  extends JpaRepository<Retweet, Long> {
	// 특정유저(AppUser  user)가 특정게시글(Post  orginalPost) 리트윗 단건조회  findBy, ※  Optional<Retweet> - By 조건절(where)
	Optional<Retweet>  findByUser_IdAndOriginalPost_Id(Long userId, Long postId);
	
	// 중복방지용 : 집계 -   특정유저(AppUser  user)가 특정게시글(Post  orginalPost) 리트윗 countBy  ※ long
	long   countByUser_IdAndOriginalPost_Id(Long userId, Long postId);
	
	// 리트윗취소 : 특정유저(AppUser  user)가 특정게시글(Post  orginalPost) 취소   deleteBy   ※ void
