...
public class UserService { 
	...
	public  UserResponseDto  updateNickname( Long userId,  String newNickname) {		
		if( appUserRepository.existsByNickname( newNickname ) ) { throw new IllegalArgumentException("이미 사용중인 닉네임입니다."); }
		// 해당유저번호 받아서 유저찾기		
		AppUser user   =  appUserRepository.findById(userId)
							.orElseThrow( ()-> new ResourceNotFoundException("사용자를 찾을 수 없습니다. ID: " +userId) );
		// 수정
		user.setNickname(newNickname); // 값변경 → 트랜잭션 종료시 장동 update   ( save 한 효과 )
		return   UserResponseDto.fromEntity(user);		
	}	
	// Update :  프로필이미지변경
	@Transactional
	...
	public void deleteById(Long userId) {
		//선댁1) 소프트딜리트 - 프로필이미지변경한 예시처럼 수정
		if( !appUserRepository.existsById(userId)  ) {	
			throw new IllegalArgumentException("삭제할 사용자가 존재하지 않습니다. ID: " +userId);  
		}
		appUserRepository.deleteById(userId);	//선댁2) 실제 db에서 삭제 ( 하드딜리트 )		
	}
