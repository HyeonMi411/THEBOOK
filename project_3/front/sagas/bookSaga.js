...
export function* fetchBestsellers() {
    try {
        const result = yield call(fetchBestsellersAPI);
        yield put(fetchBestsellersSuccess(result.data));
    } catch (err) {
        yield put(fetchBestsellersFailure(err.response?.data?.message || err.message));
    }
}
...
function* watchFetchBestsellers() { yield takeLatest(fetchBestsellersRequest.type, fetchBestsellers); }
