...
public class AiService {
	...
	public String askToGptWithContext(String context, String question) {
		...
		// choices() 자체가 null 인 경우(OpenAI 가 에러/거부 응답을 다른 형태로 준 경우 등)까지
		// 방어 - 원래는 response.choices().isEmpty() 만 확인하다가 NPE 위험이 있었음
		if (response == null || response.choices() == null || response.choices().isEmpty()) {
			return "AI 응답을 생성하지 못했습니다.";
		}

		String content = response.choices().get(0).message().content();
		// content 가 null/빈 문자열인 경우(모델이 빈 응답을 준 경우, 콘텐츠 필터에 걸린 경우 등)
		// 그대로 반환하면 화면에 아무 것도 안 보이는 것처럼 나오므로, 명확한 안내 문구로 대체
		if (content == null || content.isBlank()) {
			return "모델이 빈 응답을 반환했습니다. 질문을 조금 바꿔서 다시 시도해주세요.";
		}
		return content;
	}
}
