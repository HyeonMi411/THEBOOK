 ...
 * - Book 과 1:1 관계이며, PK를 Book.id 와 공유합니다(@MapsId). 즉 BOOK_ID 컬럼 하나가
 *   PK 이면서 동시에 Book 을 가리키는 FK 역할을 합니다.
 * - 결제 승인 시 여러 사용자가 같은 도서를 동시에 구매할 수 있어 재고차감에 동시성 문제가
 *   생길 수 있습니다. @Version(낙관적 락)으로 대응합니다 — 두 트랜잭션이 동시에 같은 재고를
 *   차감하려 하면, 먼저 커밋된 쪽만 성공하고 나중 쪽은 OptimisticLockException 이 발생해서
 *   재시도하거나 실패 처리할 수 있습니다. 결제승인처럼 즉시 확정이 필요한 임계구간은
 *   BookStockRepository.findByIdForUpdate() 의 비관적 락(FOR UPDATE)으로 추가 보강합니다.
 */
@Entity
