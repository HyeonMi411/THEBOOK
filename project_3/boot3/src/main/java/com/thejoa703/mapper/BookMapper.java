...
public interface BookMapper {
	...
	// 제목중복확인(AJAX) - 삭제된(소프트삭제) 도서는 제외하고 확인하므로, 예전에 삭제했던
	// 제목으로 다시 등록할 수 있습니다.
	...
	// 소프트 삭제 - 실제 행을 지우지 않고 DELETED 플래그만 갱신합니다. CART_ITEM/ORDER_ITEMS 가
	// BOOK_ID 를 FK 로 참조하고 있어서, 장바구니에 담겼거나 한 번이라도 주문된 도서를 하드
	// 삭제하면 FK 제약조건 위반(ORA-02292)이 발생하기 때문입니다.
	int updateDeleted(@Param("bookId") Long bookId, @Param("deleted") boolean deleted);

	// 실제 DB 행을 완전히 지웁니다. 실서비스 흐름(BookService)에서는 호출하지 않고,
	// 테스트 데이터 정리나 관리자 배치(예: 소프트삭제 후 N개월 지난 데이터 완전삭제)
	// 용도로만 사용하세요. 호출 전 재고(BOOK_STOCK)를 먼저 지워야 FK 제약을 피할 수 있고,
	// 이 도서를 참조하는 CART_ITEM/ORDER_ITEMS 가 남아있으면 여전히 실패합니다.
