 ...
 * 스키마 자동 보정 러너
 * ------------------------------------------------------------------
 * Hibernate 의 ddl-auto:update 는 "새 컬럼 추가"는 안정적으로 처리하지만,
 * "이미 존재하는 컬럼의 제약조건 완화(예: NOT NULL 해제)"는 데이터베이스/버전에
 * 따라 반영하지 못하는 경우가 흔합니다(잘 알려진 한계입니다). 그래서 이런 변경은
 * 자동생성에만 의존하지 않고, 애플리케이션 기동 시점에 이 러너가 직접 실제
 * 컬럼 상태를 확인해서 필요하면 고쳐줍니다.
 *
 * - BOOK.PUBLISH_DATE : 카카오/국립중앙도서관 자동수집 도서는 원본에 출판일 정보가
 *   없거나 파싱에 실패할 수 있어서 null 로 저장합니다(예전처럼 1900-01-01 같은
 *   매직넘버를 채우지 않습니다). 그러려면 이 컬럼이 NULL 을 허용해야 합니다.
 *
 * 매번 서버가 켜질 때마다 실행되지만, 이미 NULL 을 허용하는 상태라면 아무것도
 * 하지 않고 조용히 넘어가므로(멱등성) 여러 번 실행돼도 안전합니다. 혹시 이
 * 작업이 실패해도(예: 권한 부족) 애플리케이션 기동 자체를 막지 않도록 예외를
 * 삼키고 경고 로그만 남깁니다.
 * ------------------------------------------------------------------
 */
@Slf4j
