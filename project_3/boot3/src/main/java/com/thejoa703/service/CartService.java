...
public class CartService {
	...
	public CartResponseDto getCart(Long userId) {
		...
		List<CartItem> freshItems = cartItemMapper.findByCartId(cart.getId());
	...
	public CartResponseDto addToCart(Long userId, CartItemRequestDto dto) {
		...
		CartItem existing = cartItemMapper.findByCartIdAndBookId(cart.getId(), book.getId());
		if (existing != null) {
	...
	public CartResponseDto updateQuantity(Long userId, Long itemId, int quantity) {
		Cart cart = cartMapper.findByUserId(userId);
		if (cart == null) {
			throw new ResourceNotFoundException("장바구니가 없습니다.");
		}
		CartItem item = cartItemMapper.findById(itemId);
		if (item == null) {
			throw new ResourceNotFoundException("장바구니 항목이 없습니다. ID : " + itemId);
		}
