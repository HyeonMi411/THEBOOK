...
public class ApiNaverBook {
	...
	public List<BookDto> getBooks(String query) {
		...
		java.net.URI uri = UriComponentsBuilder.fromUriString("https://openapi.naver.com/v1/search/book.json")
				.queryParam("query", query).build().toUri();

		// 2. 설정값 : responseBody
		List<BookDto> result = new ArrayList<>();
		try {
			String responseBody = restClient.get().uri(uri).header("X-Naver-Client-Id", clientId)
					.header("X-Naver-Client-Secret", clientSecret).retrieve() // 설정내용을 http요청주고 응답 받아와
					.body(String.class);
			// 3. 값받기
...
// title, author , image	
/*
 * > GET /v1/search/book.xml?query=%EC%A3%BC%EC%8B%9D&display=10&start=1
 * HTTP/1.1 > Host: openapi.naver.com > User-Agent: curl/7.49.1 > Accept: * / *
 * > X-Naver-Client-Id: {애플리케이션 등록 시 발급받은 클라이언트 아이디 값} > X-Naver-Client-Secret:
 * {애플리케이션 등록 시 발급받은 클라이언트 시크릿 값} > 요청 예 curl
